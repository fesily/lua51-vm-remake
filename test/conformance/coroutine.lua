...
if _VERSION == "Lua 5.1" then
  local ok, err = pcall(coroutine.create, coroutine.yield)
  assert(not ok, err)
else
...
for i = 0, 30 do
  ...
  local co = coroutine.create(function() coroutine.yield(unpack(T)) end)
...
if coroutine.close then
  ...
  co = coroutine.create(function()
    local function f(depth)
      if depth > 0 then return f(depth - 1) + depth else return 0 end
    end
