  ...
  table.freeze = function(t)
    return setmetatable(t, {
      __index = function(obj, k)
        ...
        error("Attempt to index frozen table")
      end,
      __newindex = function()
        error("Attempt to index frozen table")
      end
...
checksort({ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" }, nil, "Apr", "Aug",
...
local support_nomodified_table = (function()
  local t = { 1, 2 }
...
if support_nomodified_table then
...
a = {}
for i = 1, 10 do
  a[i] = { val = math.random(100) }; setmetatable(a[i], tt);
end
table.sort(a)
check(a, tt.__lt)
check(a)
